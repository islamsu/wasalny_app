  ...
  return (
    <ScreenContainer safeAreaClassName="bg-background" edges={["top", "left", "right"]}>
      ...
      >
        ...
        <View style={[styles.nearbyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={styles.sectionHeading}><Text style={[styles.sectionTitle, { color: colors.foreground }]}>سائقين قريبين منك</Text><Text style={[styles.linkText, { color: colors.primary }]}>{nearbyDrivers.length} متاح الآن</Text></View>{nearbyDrivers.map((driver) => <Pressable key={driver.id} onPress={() => { setSelectedNearbyDriver(driver.id); setVehicle(driver.vehicle === "car" ? "car" : "toktok"); setStage("request"); }} onLongPress={() => { const driverUserId = Number(driver.id.replace(/\D/g, "")); if (driverUserId) addFavoriteMutation.mutate({ driverUserId }); }} accessibilityHint="اضغط مطولاً لإضافة السائق إلى المفضلين" style={[styles.nearbyRow, { borderTopColor: colors.border, backgroundColor: selectedNearbyDriver === driver.id ? "#E8F5EE" : colors.surface }]}><Text style={[styles.driverEta, { color: colors.primary }]}>متاح الآن</Text><View style={styles.driverCopy}><Text style={[styles.driverName, { color: colors.foreground }]}>{driver.name}</Text><Text style={[styles.driverMeta, { color: colors.muted }]}>{driver.vehicle === "car" ? "سيارة" : "توك توك"} · {driver.distanceKm.toFixed(1)} كم</Text></View><Text style={styles.driverIcon}>{driver.vehicle === "car" ? "🚗" : "🛺"}</Text><Pressable onPress={() => { const driverUserId = Number(driver.id.replace(/\D/g, "")); if (driverUserId) router.push({ pathname: "/driver-ratings", params: { driverUserId: String(driverUserId), driverName: driver.name } } as any); }} accessibilityLabel={`عرض تقييمات ${driver.name}`} style={styles.ratingShortcut}><Text style={styles.ratingShortcutText}>★</Text></Pressable></Pressable>)}</View>

        {(favoritesQuery.data ?? []).length > 0 && <View style={[styles.nearbyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={styles.sectionHeading}><Text style={[styles.sectionTitle, { color: colors.foreground }]}>السائقون المفضلون</Text><Text style={[styles.linkText, { color: colors.primary }]}>محفوظون</Text></View>{(favoritesQuery.data ?? []).map((driver) => <View key={driver.favoriteId} style={[styles.nearbyRow, { borderTopColor: colors.border }]}><Pressable onPress={() => { setVehicle(driver.vehicleType === "car" ? "car" : "toktok"); setStage("request"); }} style={styles.favoriteRequest}><Text style={[styles.driverEta, { color: colors.primary }]}>{driver.isOnline ? "متاح" : "غير متصل"}</Text><View style={styles.driverCopy}><Text style={[styles.driverName, { color: colors.foreground }]}>{driver.name ?? "سائق محفوظ"}</Text><Text style={[styles.driverMeta, { color: colors.muted }]}>{driver.vehicleType === "car" ? "سيارة" : "توك توك"}</Text></View><Text style={styles.driverIcon}>{driver.vehicleType === "car" ? "🚗" : "🛺"}</Text><Pressable onPress={() => router.push({ pathname: "/driver-ratings", params: { driverUserId: String(driver.driverUserId), driverName: driver.name ?? "السائق" } } as any)} accessibilityLabel="عرض سجل تقييمات السائق" style={styles.ratingShortcut}><Text style={styles.ratingShortcutText}>★</Text></Pressable></Pressable><Pressable onPress={() => removeFavoriteMutation.mutate({ driverUserId: driver.driverUserId })} style={styles.favoriteRemove}><Text style={[styles.linkText, { color: colors.error }]}>إزالة</Text></Pressable></View>)}</View>}
...
function OffersSheet({ colors, offers, onCancel, onSelect }: any) {
...
const styles = StyleSheet.create({
  ...
  sectionSubtitle: { fontSize: 12, marginTop: 4, textAlign: "right" }, nearbyCard: { borderWidth: 1, borderRadius: 20, padding: 14, gap: 10 }, linkText: { fontSize: 10, fontWeight: "800" }, favoriteRequest: { flex: 1, flexDirection: "row-reverse", alignItems: "center", gap: 10 },   ratingShortcut: { paddingHorizontal: 8, paddingVertical: 6 },
  ratingShortcutText: { color: "#F4B740", fontSize: 19 },
  favoriteRemove: { paddingHorizontal: 6, paddingVertical: 8 }, nearbyRow: { borderTopWidth: 1, paddingTop: 11, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, driverIcon: { fontSize: 25 }, driverName: { fontSize: 13, fontWeight: "800", textAlign: "right" }, driverMeta: { fontSize: 10, marginTop: 3, textAlign: "right" }, driverCopy: { flex: 1, alignItems: "flex-end" }, driverEta: { fontSize: 13, fontWeight: "800" },
  wave: { fontSize: 25 },
  actions: { gap: 10 },
  ...
  offerCard: { borderWidth: 1, borderRadius: 16, padding: 12, marginTop: 12, flexDirection: "row-reverse", alignItems: "center", gap: 8 }, offerCopy: { flex: 1, alignItems: "flex-end" }, offerDriver: { fontSize: 13, fontWeight: "800" }, offerMeta: { fontSize: 10, marginTop: 3, textAlign: "right" }, offerPrice: { fontSize: 14, fontWeight: "800" }, offerSelect: { borderRadius: 10, paddingHorizontal: 10, paddingVertical: 8 }, offerSelectText: { color: "#FFFFFF", fontSize: 10, fontWeight: "800" },
