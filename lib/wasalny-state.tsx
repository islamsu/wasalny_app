...
export type DriverAccountStatus = "active" | "frozen" | "suspended";
...
export function WasalnyStateProvider({ children }: { children: ReactNode }) {
  const [currentUser, setCurrentUser] = useState<{ role: UserRole; name: string } | null>({ role: "family", name: "العائلة" });
  const [subscriptionStatus, setSubscriptionStatus] = useState<SubscriptionStatus>("pending");
  const [driverOnline, setDriverOnline] = useState(false);
  const [driverAccountStatus, setDriverAccountStatus] = useState<DriverAccountStatus>("active");
  ...
  const login = (role: UserRole, name?: string) => setCurrentUser({ role, name: name ?? (role === "admin" ? "المشرف" : role === "driver" ? "السائق" : "العائلة") });
  const logout = () => { setCurrentUser(null); setDriverOnline(false); };
  const toggleDriverOnline = () => { if (subscriptionStatus === "approved" && driverAccountStatus === "active") setDriverOnline((current) => !current); };
